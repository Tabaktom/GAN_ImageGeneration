...
def discriminator(x, drop_out):
    ...
    # 3rd hidden layer
    w2 = tf.get_variable('D_w2', [h1.get_shape()[1], 256], initializer=w_init)#256
    b2 = tf.get_variable('D_b2', [256], initializer=b_init)#1024
    h2 = tf.nn.leaky_relu(tf.matmul(h1, w2) + b2)
...
show_train_hist(train_hist, save=True, path='MNIST_GAN_results/MNIST_GAN_train_hist_Normal.png')
images = []
sess.close()
